...
export class DocumentService {
  private _files: BehaviorSubject<PatientFile[]> = new BehaviorSubject<PatientFile[]>([
    {
      fileID: 0,
  ...
  getPatientFiles(options: FileRequestOptions): Observable<PatientFile[]> {
  ...
  updatePatientFileGroupType(fileToUpdate: PatientFile, type: FileGroupType, options: FileRequestOptions): Observable<PatientFile[]> {
  ...
  updatePatientFileDate(fileToUpdate: PatientFile, date: Date, options: FileRequestOptions): Observable<PatientFile[]> {
    const files = this._files.value;
    const index = files.findIndex(file => file === fileToUpdate);
